...
flistservice.Url = string.Format("http://{0}:{3}/{2}/WS/{1}/Page/Flist",
        ...
        public void TFarmers(string data)
        {
            ...
            try
            {
                ...
                vservice.Timeout = 600000;
                v = JsonConvert.DeserializeObject<Vendor.Vendor>(data);
                var f = vservice.ReadMultiple(new Vendor.Vendor_Filter[] { new Vendor.Vendor_Filter { Criteria = "Farmer", Field = Vendor.Vendor_Fields.Creditor_Type }, new Vendor.Vendor_Filter { Criteria = v.No, Field = Vendor.Vendor_Fields.Transporter } }, null, 30000).ToList();
                //foreach (var vv in f)
                //{
                //    double cumm = 0;
                //    var c = GetCollection(vv.No);
        ...
        [WebMethod]
        [ScriptMethod(ResponseFormat = ResponseFormat.Json)]
        public void GetCollectionstransporter(string data)
        {
            CUtilities.LogEntryOnFile(data);
            string response = string.Empty;
            getdata v = null;
            try
            {
                ...
                string[] date2 = v.LastDate.Split(new char[] { '-' });
                DateTime d2 = new DateTime(int.Parse(date2[2]), int.Parse(date2[1]), int.Parse(date2[0]));
                var c = Collservice.ReadMultiple(new DataCollection.Collection_Filter[] { new DataCollection.Collection_Filter { Criteria = v.route, Field = DataCollection.Collection_Fields.Transporter }, new DataCollection.Collection_Filter { Criteria = d.Date.ToShortDateString() + ".." + d2.Date.ToShortDateString(), Field = DataCollection.Collection_Fields.Collections_Date } }, null, 0).ToList();
                foreach (var cc in c)
                {
                    if (string.IsNullOrEmpty(cc.Date))
                    {
        ...
        public void Collections(string data)
        {
            ...
            try
            {
                ...
                var c = Collservice.ReadMultiple(new DataCollection.Collection_Filter[] { new DataCollection.Collection_Filter { Criteria = collection.Farmers_Number, Field = DataCollection.Collection_Fields.Farmers_Number }, new DataCollection.Collection_Filter { Criteria = collection.Collections_Date.ToShortDateString(), Field = DataCollection.Collection_Fields.Collections_Date }, new DataCollection.Collection_Filter { Criteria = collection.Collection_Number, Field = DataCollection.Collection_Fields.Collection_Number } }, null, 0).FirstOrDefault();

                //var c = Collservice.Read(collection.Farmers_Number,collection.Collections_Date,  collection.Collection_Number);
                if (c == null)
