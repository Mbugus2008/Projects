...
public class DB extends SQLiteOpenHelper {
    ...
    public ArrayList<summaries.collectiondates> getcollectiondates() {
       ...
       // Cursor res = db.query(true, t.Transactions_TABLE_NAME, new String[]{t.Date}, null, null, null, null, t.OTTN + " DESC", null);
        Cursor res = db.rawQuery("select Date,Sum(Amount) as Total,count(Document_No) as Count from transactions  group by Date order by  substr(Date,7)||substr(date,4,2)||substr(Date,1,2)  desc",null);

        res.moveToFirst();
        while (res.isAfterLast() == false) {
            summaries.collectiondates f = new summaries.collectiondates();
            f.date = res.getString(res.getColumnIndex("Date"));
            f.Count = res.getInt(res.getColumnIndex("Count"));
            f.Total = res.getDouble(res.getColumnIndex("Total"));
            array_list.add(f);
            res.moveToNext();
        }
    ...
    public ArrayList<transaction> gettransbydate(String date) {
        ...
        //hp = new HashMap();
        SQLiteDatabase db = this.getReadableDatabase();
        Cursor res = db.rawQuery("select *,(select Name from types tt where tt.Code = t.Type ) as typename from " + t.Transactions_TABLE_NAME + " t Where " + t.Date + " =? order by Time desc", new String[]{date + ""});
        res.moveToFirst();
        while (res.isAfterLast() == false) {
            ...
            f.Type = res.getString(res.getColumnIndex(t.Type));
            f.Group = res.getString(res.getColumnIndex(t.Group));
            f.typename = res.getString(res.getColumnIndex("typename"));
            res.moveToNext();
            array_list.add(f);
        }
        res.close();
...
//        contentValues.put(m.updated, f.updated);
    ...
    public agent getagent(String agent) {
        ...
        SQLiteDatabase db = this.getReadableDatabase();
        Cursor res = db.rawQuery("select * from " + a.agent_Table + " Where "+a.Status +" = 2 and " + a.Code + " =?", new String[]{agent + ""});
        if (res.getCount() > 0) {
            res.moveToFirst();
            f = new agent();
...
//        }
    ...
    public List<vehicles> getvehicleslist(  String c) {
        SQLiteDatabase db = this.getReadableDatabase();
        ArrayList<vehicles>  ff = new ArrayList<vehicles>();
        String q = "select * from " + v.vehicles_Table + " where " + v.Vehicle_Number + " in (" + c + ")";
