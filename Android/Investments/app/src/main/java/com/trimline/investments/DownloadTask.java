...
public class DownloadTask {
    ...
    public DownloadTask(Context context, ProgressBar p, String downloadUrl) {
        this.context = context;
        this.progressBar = p;
        this.downloadUrl = downloadUrl;
        String[] f = downloadUrl.split("/");

//        downloadFileName = downloadUrl.replace(Utils.mainUrl, "");//Create file name by picking download file name from URL
        downloadFileName = f[f.length-1] ;//Create file name by picking download file name from URL
    ...
    private class DownloadingTask extends AsyncTask<Void, Void, Void> {
        ...
        protected void onPostExecute(Void result) {
            try {
                if (outputFile != null) {
                    progressBar.setVisibility(View.GONE);//.setEnabled(true);
...
//                    }, 3000);
                ...
                e.printStackTrace();
                Toast.makeText(context, "Download Failed - "+ e.getLocalizedMessage(), Toast.LENGTH_SHORT).show();
                progressBar.setVisibility(View.GONE);//.setEnabled(true);
