...
f = new user();
    ...
    public ArrayList<Collection> getAllCollection() {
        ...
        while (res.isAfterLast() == false) {
            ...
            f.More =  (res.getString(res.getColumnIndex(More)));
            array_list.add(f);
            res.moveToNext();
        }
        res.close();
...
//        });
    ...
    public double gettotalcollbydates(String date) {
      double total = 0.0;
        SQLiteDatabase db = this.getReadableDatabase();
        Cursor res = db.rawQuery("select sum("+Kg_Collected +") as total from " + Collection_TABLE_NAME + " where Collections_Date = '"+ date+"' and  "+ status +" <> 'Reversed'" , null);
        if(res.moveToFirst())
        {
