...
f = new user();
    ...
    public List<Summaries.Bydate> getdatesBetween(String from, String to) {
        List<Summaries.Bydate> list = new ArrayList<>();
        SQLiteDatabase db = this.getReadableDatabase();

        Cursor cursor = db.rawQuery("SELECT DISTINCT "+ Collections_Date +" FROM "+Collection_TABLE_NAME +" WHERE  "+ Collections_Date +"  BETWEEN ? AND ? ORDER BY  "+ Collections_Date +"  DESC", new String[]{from, to});
        if (cursor.moveToFirst()) {
            do {
                Summaries.Bydate b = new Summaries.Bydate();
                b.Date = cursor.getString(0);
                list.add(b);
    ...
    public List<Summaries.Bydate> getPagedDates(int page, int pageSize, String date1,String date2) {
        List<Summaries.Bydate> allDates;
        if ((date1 != null) && (date2 !=null)) {
       allDates= getdatesBetween(date1,date2);

       }
       else{
      allDates = getdates();}

       // existing method to get all dates
        int fromIndex = page * pageSize;
        int toIndex = Math.min(fromIndex + pageSize, allDates.size());

        if (fromIndex >= allDates.size()) {
            return new ArrayList<>();
        }

        return allDates.subList(fromIndex, toIndex);
    }
...
//        });
    ...
    public int getcountcollbydates(String date) {
        int total = 0;
        SQLiteDatabase db = this.getReadableDatabase();
        Cursor res = db.rawQuery("select  count(distinct Farmers_Number) as c from " + Collection_TABLE_NAME + " where Collections_Date = '"+ date+"' and  "+ status +" <> 'Reversed'" , null);
        if(res.moveToFirst())
        {
            total =  res.getInt(0);
        }
