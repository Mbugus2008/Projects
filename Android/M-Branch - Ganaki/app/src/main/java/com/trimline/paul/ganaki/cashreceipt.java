...
public class cashreceipt extends AppCompatActivity {
    ...
    TextView membername, id, totalrec, otherdetails, loanbalance, operationcost;
        ...
        memberno.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long iid) {
                ...
                if (veh != null) {
                    ...
                    new vehicledata().execute(veh.Vehicle_Number);
                }
                if (f == null) {
        ...
        find.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (veh != null) {
                    ...
                    otherdetails.setText("Penalty: " + String.valueOf(veh.Penalty));
...
public static void createfile(transaction c)
{
    ...
    private class memberdata extends AsyncTask<String, String, List<vehicles>> {
        @Override
        protected void onPreExecute() {
        }
        ...
        @Override
        protected List<vehicles> doInBackground(String... params) {
            //publishProgress("Getting logins");
            List<vehicles> results = null;
            String result = null;
            try {
                Gson g = new Gson();
                result = JsonParser.postjson("member_Vehicles", "memberno", params[0]);
                ...
                if (results != null) {
                    try {
                       ...
                       db.deletevehiclesforMember(params[0]);
    ...
    private class vehicledata extends AsyncTask<String, String, vehicles> {
        @Override
        protected void onPreExecute() {
        }
        ...
        @Override
        protected vehicles doInBackground(String... params) {
            //publishProgress("Getting logins");
            vehicles results = null;
            String result = null;
            try {
                Gson g = new Gson();
                result = JsonParser.postjson("Vehiclesdetails", "vehicleno", params[0]);
                Type localType = new TypeToken<vehicles>() {
                }.getType();
                results = new GsonBuilder().setDateFormat("yyyy-MM-dd").create().fromJson(result, localType);
                if (results != null) {
                    try {
                       db.updatevehicles(results);
                    } catch (Exception ex) {
                        ex.printStackTrace();
                    }
                } else {
        ...
        @Override
        protected void onPostExecute(vehicles res) {
            try {
                StringBuilder s = new StringBuilder();
                DateFormat df = new DateFormat();
                s.append(String.format("Last Repayment: <b>%s</b>",df.format("yyyy-MM-dd", res.Last_Payment_Line)));

                otherdetails.setText(Html.fromHtml(s.toString()));
                otherdetails.setTextColor(Color.GREEN);

              long days =  TimeUnit.DAYS.convert((new Date().getTime()- res.Last_Payment_Line.getTime()),TimeUnit.MILLISECONDS);
                SimpleDateFormat formatter = new SimpleDateFormat("dd/MM/yyyy");

                Date today = new Date();
                Date todayWithZeroTime = formatter.parse(formatter.format(today));

                if ( todayWithZeroTime.after(res.Last_Payment_Line)) {
                    s = new StringBuilder();
                    s.append(String.format("Last Repayment: <b>%s</b> (%s day(s) Ago)",df.format("yyyy-MM-dd", res.Last_Payment_Line), days));
                    otherdetails.setText(Html.fromHtml(s.toString()));
                    otherdetails.setTextColor(Color.RED);
                }
