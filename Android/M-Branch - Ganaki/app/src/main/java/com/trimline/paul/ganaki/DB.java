...
public class DB extends SQLiteOpenHelper {
    ...
    public static class t {
        ...
        static transaction get_t(Cursor res) {
            ...
            f.Fuel = res.getDouble(res.getColumnIndex(t.Fuel));
        ...
        static transaction get_t_summary(Cursor res) {
            transaction f = new transaction();
            f.Account_No = res.getString(res.getColumnIndex(t.Account_No));
            f.Account_Name = res.getString(res.getColumnIndex(t.Account_Name));
            f.Amount = res.getDouble(res.getColumnIndex(t.Amount));
        ...
        static transaction get_t_daily_summary(Cursor res) {
            transaction f = new transaction();
            ...
            f.Welfare = res.getDouble(res.getColumnIndex(t.Welfare));
            f.Stage = res.getDouble(res.getColumnIndex(t.Stage));
            f.count = res.getInt(res.getColumnIndex("Counts"));
        ...
        public static ContentValues values(transaction f) {
            ...
            contentValues.put(t.Fuel, f.Fuel);
    ...
    public boolean inserttrans(transaction f) {
        try {
            ...
            db.insertWithOnConflict(t.Transactions_TABLE_NAME, null, t.values(f), SQLiteDatabase.CONFLICT_REPLACE);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return true;
    ...
    public boolean updatetrans(transaction f) {
        SQLiteDatabase db = this.getWritableDatabase();
        db.update(t.Transactions_TABLE_NAME, t.values(f), t.Document_No + " = ? ", new String[]{f.Document_No});
        return true;
    }
    ...
    public transaction gettransbydocument(String doc) {
        ...
        array_list = t.get_t(res);

        res.close();
        return array_list;
    }
    public ArrayList<transaction> gettransbydate_daily(String date) {
        ArrayList<transaction> array_list = new ArrayList<transaction>();
        //hp = new HashMap();
        SQLiteDatabase db = this.getReadableDatabase();
        Cursor res = db.rawQuery("select Count(Date)As Counts, Date, Sum(Loan) As Loan,Sum(Shares) as Shares,Sum(Welfare) as Welfare,sum(Operation)as Operation,sum(Stage) as Stage,sum(Court_Bond) as Court_Bond,Sum(Amount) as Amount,Sum(Savings) as Savings from " + t.Transactions_TABLE_NAME + " Where " + t.Date + " =? group by Date", new String[]{date + ""});
        Log.i("Rows", String.valueOf( res.getCount()));
        res.moveToFirst();
        while (res.isAfterLast() == false) {
            array_list.add(  t.get_t_daily_summary(res));
            res.moveToNext();
        }
        res.close();
        return array_list;
    }
    public ArrayList<transaction> gettransbydate_2(String date) {
        ArrayList<transaction> array_list = new ArrayList<transaction>();
        //hp = new HashMap();
        SQLiteDatabase db = this.getReadableDatabase();
        Cursor res = db.rawQuery("select Date, Account_No,Account_Name,Loan_No, Sum(Loan) As Loan,Sum(Shares) as Shares,Sum(Welfare) as Welfare,sum(Operation)as Operation,sum(Stage) as Stage,sum(Court_Bond) as Court_Bond,Sum(Amount) as Amount,Sum(Savings) as Savings from " + t.Transactions_TABLE_NAME + " Where " + t.Date + " =? group by Date, Account_No,Account_Name,Loan_No", new String[]{date + ""});
        Log.i("Rows", String.valueOf( res.getCount()));
        res.moveToFirst();
        while (res.isAfterLast() == false) {


            array_list.add(  t.get_t_summary(res));
            res.moveToNext();
        }
        res.close();
        return array_list;
...
//            f.Type = res.getString(res.getColumnIndex(t.Type));
//            f.Group = res.getString(res.getColumnIndex(t.Group));
            array_list.add(t.get_t(res)); res.moveToNext();
...
//        }
    ...
    public boolean updatevehicles(vehicles f) {
        SQLiteDatabase db = this.getWritableDatabase();
        db.update(v.vehicles_Table, v.values(f), v.Vehicle_Number + " = ? ", new String[]{f.Vehicle_Number});
        return true;
    }
