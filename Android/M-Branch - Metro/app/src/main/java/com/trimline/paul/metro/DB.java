...
public class DB extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        db.execSQL(
                "create table " + m.member_Table + "" +
                        ...
                        m.updated + " boolean," +
                        m.E_Mail + " text," +
                        m.Last_update_Loan + " text," +
                        m.Last_update_savings + " text," +
                        m.Last_update_xmas + " text," +
                        m.Un_allocated_Funds + " float," +
                        m.Outstanding_Balance + " float," +
                        m.Repayment + " float," +
                        m.Operation + " float," +
                        m.Penalty + " float," +
                        m.Loan_Arrears + " float," +
                        m.dailyrepayment + " float," +
                        m.Savings + " float," +
                        m.Xmas + " float," +
                        m.Group + " text)");
        db.execSQL(
        ...
        db.execSQL(
                "create table " + agt.Agenttypetable + "" +
                        "(" + agt.Agent + " text ," +
                        agt.TransactionType + " text ," +
                        agt.Desc + " text," +
                        " PRIMARY KEY ( "+agt.Agent+ ", " + agt.TransactionType +"))");
    }
    ...
    static class agt {
        ...
        public static final String Agenttypetable = "AgentTypeName";
        public static final String Agenttypetabletmp = "AgentTypeNametmp";
        public static ContentValues values(AgentTypes l) {
            ContentValues contentValues = new ContentValues();
            contentValues.put(Agent, l.Agent);
            contentValues.put(TransactionType, l.Transaction_Type);
            contentValues.put(Desc, l.Desc);
    ...
    static class v {
        ...
        public static ContentValues values(vehicles vv) {
            ...
            contentValues.put(Fleet_No, vv.Fleet_No);
    ...
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        ...
        try {
            ...
            List<String> atypes = null;
            if (tableexists(db, agt.Agenttypetable)) {
                res = db.query(agt.Agenttypetable, null, null, null, null, null, null);
                cols = res.getColumnNames();
                atypes = new ArrayList<String>(Arrays.asList(cols));
            }
            ...
            db.execSQL("Drop table if exists " + agt.Agenttypetabletmp);
            ...
            if (tableexists(db, agt.Agenttypetable))
                db.execSQL("ALTER table " + agt.Agenttypetable+ " RENAME TO '" + agt.Agenttypetabletmp + "'");
            ...
            if ((tableexists(db, agt.Agenttypetable)) && (tableexists(db, agt.Agenttypetabletmp)))
                db.execSQL(String.format("INSERT INTO %s (%s) SELECT %s from %s",  agt.Agenttypetable, agtcol, agtcol, agt.Agenttypetabletmp));
            ...
            if (tableexists(db, agt.Agenttypetabletmp))
                db.execSQL(" DROP table '" + agt.Agenttypetabletmp + "'");
    ...
    public boolean inserttrans(transaction f) {
        try {
            ...
            contentValues.put(t.Amount, f.getAmount());
            ...
            contentValues.put(t.Recovery, f.Recovery);
    ...
    public ArrayList<tsummary> gettranssummarybydate(String date) {
        ArrayList<tsummary> array_list = new ArrayList<tsummary>();
        //hp = new HashMap();
        SQLiteDatabase db = this.getReadableDatabase();
       // Cursor res = db.rawQuery("select * from " + t.Transactions_TABLE_NAME + " Where " + t.Date + " =?", new String[]{date + ""});
        ...
        while (res.isAfterLast() == false) {
            ...
            tsummary f = new tsummary();
            f.Type = res.getString(res.getColumnIndex("Type"));

            f.Amount = res.getDouble(res.getColumnIndex("Amount"));
...
//        contentValues.put(m.Loan_Arrears, f.Loan_Arrears);
//        contentValues.put(m.dailyrepayment, f.dailyrepayment);
//        contentValues.put(m.updated, f.updated);

        db.insertWithOnConflict(m.member_Table, null, m.values(f), SQLiteDatabase.CONFLICT_REPLACE);
        return true;
    }
...
//        contentValues.put(m.Operation, f.Operation_Cost);
//        contentValues.put(m.updated, f.updated);
        db.update(m.member_Table, m.values(f), m.No + " =?", new String[]{f.No});
        return true;
    }
    ...
    public ArrayList<member> getupdatedmember() {
        ...
        res.moveToFirst();
        while (res.isAfterLast() == false) {
            member f = m.getvalues(res);// new member();
...
//            f.updated = (res.getInt(res.getColumnIndex(m.updated)) == 0 ? false : true);
    ...
    public ArrayList<types> gettypes() {
        ...
        SQLiteDatabase db = this.getReadableDatabase();
        Cursor res = db.rawQuery("select * from " + tt.Types_Table + " where Code <>'' AND Active =1 order by lOrder", null);
        ...
        while (res.isAfterLast() == false) {
            ...
            f.Type = res.getInt(res.getColumnIndex(tt.Type));
    ...
    public ArrayList<AgentTypes> getagenttypes() {
        ArrayList<AgentTypes> array_list = new ArrayList<AgentTypes>();
        SQLiteDatabase db = this.getReadableDatabase();
        Cursor res = db.rawQuery("select * from " + agt.Agenttypetable, null);
        res.moveToFirst();
        while (res.isAfterLast() == false) {
            AgentTypes f = new AgentTypes();

            f.Agent = res.getString(res.getColumnIndex(agt.Agent));
            f.Transaction_Type = res.getString(res.getColumnIndex(agt.TransactionType));
            f.Desc = res.getString(res.getColumnIndex(agt.Desc));
...
//        }
    ...
    public ArrayList<vehicles> getvehicles() {
        ...
        while (res.isAfterLast() == false) {
            if (res.getString(res.getColumnIndex(v.Code))!=null) {
                ...
                f.Fleet_No = res.getString(res.getColumnIndex(v.Fleet_No));
    ...
    public void deletevehiclesforMember(String member) {
        SQLiteDatabase db = this.getWritableDatabase();
        db.delete(v.vehicles_Table, v.Code + " = ? ", new String[]{member});
    }
    ...
    public vehicles getvehiclebyfleet(String c) {
        SQLiteDatabase db = this.getReadableDatabase();
        vehicles f = null;
            Cursor res = db.rawQuery("select * from " + v.vehicles_Table + " where " + v.Fleet_No + "= '" + c + "'", null);
        if (res.getCount() > 0) {
            res.moveToFirst();
            f = new vehicles();
            f.Code = res.getString(res.getColumnIndex(v.Code));
    ...
    public List<vehicles> getvehiclesbyfleet(  String c) {
        SQLiteDatabase db = this.getReadableDatabase();
        ArrayList<vehicles>  ff = new ArrayList<vehicles>();
        String q = "select * from " + v.vehicles_Table + " where " + v.Fleet_No + " in (" + c + ")";
    ...
    public boolean inserttypeagent(AgentTypes f) {
        SQLiteDatabase db = this.getWritableDatabase();
        db.insertWithOnConflict(agt.Agenttypetable, null, agt.values(f), SQLiteDatabase.CONFLICT_REPLACE);
    ...
    static class m {
        ...
        public static final String Loan_Arrears = "Loan_arrears";
        public static final String dailyrepayment = "daily_repayment";

        public static final String Savings = "Savings";
        public static final String Xmas = "Xmas";
        public static final String Last_update_savings = "Last_update_savings";
        public static final String Last_update_xmas = "Last_update_xmas";
        public static final String Last_update_Loan = "Last_update_Loan";




        public static ContentValues values(member f) {
            ...
            contentValues.put(m.dailyrepayment, f.dailyrepayment);
            contentValues.put(m.updated, f.updated);
            contentValues.put(m.Savings, f.Savings);
            contentValues.put(m.Xmas, f.Xmas);
            contentValues.put(m.Last_update_Loan, f.Last_update_Loan);
            contentValues.put(m.Last_update_savings, f.Last_update_savings);
            contentValues.put(m.Last_update_xmas, f.Last_update_xmas);


            return contentValues;
        }

        public static member getvalues(Cursor res) {
            ...
            f.Outstanding_Penalty = res.getDouble(res.getColumnIndex(m.Penalty));
            f.Operation_Cost = res.getDouble(res.getColumnIndex(m.Operation));
            f.Loan_Arrears = res.getDouble(res.getColumnIndex(m.Loan_Arrears));
            f.dailyrepayment = res.getDouble(res.getColumnIndex(m.dailyrepayment));
            f.ID_No = res.getString(res.getColumnIndex(m.ID_No));
            f.updated = (res.getInt(res.getColumnIndex(m.updated)) != 0);

            f.Savings = res.getDouble(res.getColumnIndex(m.Savings));
            f.Xmas = res.getDouble(res.getColumnIndex(m.Xmas));
            f.Last_update_Loan = res.getString(res.getColumnIndex(m.Last_update_Loan));
            f.Last_update_savings = res.getString(res.getColumnIndex(m.Last_update_savings));
            f.Last_update_xmas = res.getString(res.getColumnIndex(m.Last_update_xmas));
    ...
    }public Integer deleteAgenttypes() {
        SQLiteDatabase db = this.getWritableDatabase();
        return db.delete(agt.Agenttypetable, null, null);
    }
}
