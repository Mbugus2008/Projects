...
namespace MobiService
{
    ...
    public class Mobile : System.Web.Services.WebService
    {
        ...
        M_SACCO_Webservice.Loan_Products.Loan_Products_Service Loan_Products_Service = new M_SACCO_Webservice.Loan_Products.Loan_Products_Service();
        ...
        public Mobile()
        {
            ...
            Loan_Products_Service.Url = String.Format("http://{0}:{3}/{2}/WS/{1}/Page/Loan_Products", ss.Serverip, ss.Companyname, ss.Instance, ss.Port);
            Loan_Products_Service.PreAuthenticate = true;
            Loan_Products_Service.Credentials = cd;
        ...
        public Result Transactions(M_SACCO_Webservice.MobileTrans.MobileTransactions t)
        {
            ...
            try
            {
                Logging.Logging.CreateXML(string.Format("{0}- In - {1}\n{2}",DateTime.Now, t.Document_No, t));
                ...
                switch (t.Transaction_Type)
                {
                    ...
                    case 21://e loan
                        ...
                        var pending = mobile.ReadMultiple(new M_SACCO_Webservice.MobileTrans.MobileTransactions_Filter[] {
                            new M_SACCO_Webservice.MobileTrans.MobileTransactions_Filter { Criteria = "Pending Posting", Field = M_SACCO_Webservice.MobileTrans.MobileTransactions_Fields.Status },
                            new M_SACCO_Webservice.MobileTrans.MobileTransactions_Filter { Criteria = t.Account_No, Field = M_SACCO_Webservice.MobileTrans.MobileTransactions_Fields.Account_No },
                            new M_SACCO_Webservice.MobileTrans.MobileTransactions_Filter { Criteria = "21|12", Field = M_SACCO_Webservice.MobileTrans.MobileTransactions_Fields.Transaction_Type } }, null, 0);
                        if (pending.Count() > 0)
                            throw new Exception(String.Format("MBARAKA is unable to process your request because a similar transaction is currently underway. Please wait while we complete your initial request."));
                        ...
                        var lp = Loan_Products_Service.Read("M-BARAKA");
                        if (lp!=null )
                            if (t.Amount < lp.Min_Loan_Amount)
                                throw new Exception(String.Format("You cannot apply for a loan less than {0}", lp.Min_Loan_Amount));
                        break;

                    case 16://Patalipa
                        aap = app.ReadMultiple(new M_SACCO_Webservice.Applicatons.MobileApplications_Filter[] { new M_SACCO_Webservice.Applicatons.MobileApplications_Filter { Criteria = t.Mobile_No, Field = M_SACCO_Webservice.Applicatons.MobileApplications_Fields.MPESA_Mobile_No } }, null, 0).FirstOrDefault();
                        if (aap == null)
                            throw new Exception("Account Not found");
                        t.Account_No = aap.No;
            ...
            finally
            {
                try
                {
                    ...
                    tran.Status = M_SACCO_Webservice.MobileTrans.Status.Pending_Posting;
            ...
            Logging.Logging.CreateXML(string.Format("{0}- Out -{1}\n{2}", DateTime.Now, t.Document_No, r));
            return r;
        }
        [WebMethod]
