    ...
    public partial class Collection : Form
    {
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            try
            {
                using (var db = new AutoweighEntities())
                {
                    ...
                    if (Convert.ToDouble(this.txtkg.Text) < 1) { 
                    errorProvider1.SetError(txtkg, "Kg required");
                    return;
                }
                    ...
                    if (ff !=null)
                    ff.Cumulative += c.Kg__Collected;
                    c.Cumm = (double)ff.Cumulative + unposted(ff);
        ...
        void printDocument_PrintPage(object sender, PrintPageEventArgs e)
        {
            try
            {
                ...
                str = "Time: ";
                space = padspace(e.Graphics.MeasureString(space, font).Width, (ps - (e.Graphics.MeasureString(str, font).Width + e.Graphics.MeasureString(c.Collection_time.ToString("hh:mm"), font).Width)));

                e.Graphics.DrawString(str + space + c.Collection_time.ToString("hh:mm"), font, solidBrush, ml, y);
                y += e.Graphics.MeasureString(str, font).Height + 3;
        ...
        private void txtfarmer_Leave(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                viewcummulative.Text = (f.Cumulative + unposted(f)) .ToString();

                loadcollections();
            }
        }

        private double unposted(Farmer f)
        {
            double unp = 0;
            try
            {
                using (var db = new AutoweighEntities())
                {
                    unp = (double)db.Daily_Collections_Details.Where(o => o.Farmers_Number == f.No && o.Sent == false).Sum(o => o.Kg__Collected);
                }
            }
            catch (Exception ex) { Logging.Logging.ReportError(ex); }
            return unp;
            ...
            try
            {
                using (var db = new AutoweighEntities())
                {
                  db.Daily_Collections_Details.Load();
                    var cumm = db.Daily_Collections_Details.Local.ToBindingList().Where(o => o.Farmers_Number == txtfarmer.Text && o.Cancelled == "No");
                    daily_Collections_DetailsBindingSource.DataSource = cumm;
                    viewcummulative.Text = "Cummulative Kgs: " + (f.Cumulative + Convert.ToDouble(txtkg.Text));
                }
            }
            catch (Exception ex) { }
        }
                ...
                using (var db = new AutoweighEntities())
                {
                    ...
                    if (d == DialogResult.Yes)
                    {
                        ...
                        if (coll != null)
                        {
                            ...
                            if (ff != null)
                            {
                                ff.Cumulative -= coll.Kg__Collected;
                                ct.Cumm = (double)ff.Cumulative;
                            }
       ...
       (e.KeyChar != '.'))
        ...
        private void viewfarmername_Click(object sender, EventArgs e)
        {
