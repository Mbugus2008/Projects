...
namespace Weigh
{
    public partial class WeighMain : Form
    {
        ...
        private void Weigh_Load(object sender, EventArgs e)
        {
            ...
            t.Start();
            AutoweighEntities.Factory_Name = "GAKUYU FCS LIMITED";
                    ...
                    if (e.Result.Count() > 0)
                        foreach (var farmer in e.Result)
                        {
                            ...
                            if (f == null)
                            {
                                f = new Farmer();
                                f.No = farmer.No;
                                AutoweighEntities.Db.Farmers.Add(f);
                            }
                            SetText("Farmers(" + farmer.Name + ")");
                            f.Name = farmer.Name;
                            f.Phone = farmer.Phone_No;
                            f.Cumulative = (double)farmer.Cummulative;
                           db.SaveChanges(); }
                        }
            }
            catch (Exception ex)
            ...
            try
            {
                ...
                using (var db = new AutoweighEntities())
                {
                    ...
                    foreach (Daily_Collections_Detail c in cc)
                    {
                        ...
                        nc.Collections_Time = c.Collection_time;
                        ...
                        {                           
                                ...
                                if (cc != null)
                                    ...
                                    {                                        
                                        cc.Sent = true;
                                        cc.Comments = "";
                                        SetText(c.Collection_Number + " Updated");
                                    }
                                    else
                                    {
                                        cc.Comments = c.desc;
