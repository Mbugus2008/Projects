...
namespace Msacco
{
    ...
    public class Mobile : WebService
    {
        ...
        private Mbranch.MBranch MBranch = new  Mbranch.MBranch();
        private dcs_account_details.DCS_Account_Details_Service dcs_Account_Details = new dcs_account_details.DCS_Account_Details_Service();
        private NetworkCredential cd;
        private MobileTransactions.Mobile_Transactions_Service Mobile_Transactions_Service = new MobileTransactions.Mobile_Transactions_Service();
        ...
        public Mobile()
        {
            ...
            this.MBranch.Url = string.Format("http://{0}:{3}/{2}/WS/{1}/Codeunit/MBranch", (object)this.ss.server, (object)this.ss.Companyname, (object)this.ss.Instance, (object)this.ss.Port);
            this.MBranch.PreAuthenticate = true;
            this.MBranch.Credentials = (ICredentials)this.cd;

            this.dcs_Account_Details.Url = string.Format("http://{0}:{3}/{2}/WS/{1}/Page/DCS_Account_Details", (object)this.ss.server, (object)this.ss.Companyname, (object)this.ss.Instance, (object)this.ss.Port);
            dcs_Account_Details.PreAuthenticate = true;
            dcs_Account_Details.Credentials = (ICredentials)this.cd;

            this.Mobile_Transactions_Service.Url = string.Format("http://{0}:{3}/{2}/WS/{1}/Page/Mobile_Transactions", (object)this.ss.server, (object)this.ss.Companyname, (object)this.ss.Instance, (object)this.ss.Port);
            Mobile_Transactions_Service.PreAuthenticate = true;
            Mobile_Transactions_Service.Credentials = (ICredentials)this.cd;
        ...
        public loans LoanRequest(loans loan)
        {
            try
            {
                ...
                //int num = this.Msacco.Advance(loan.Document_No, loan.account.Account_No, loan.Amount);
                //loan.results = num.ToString();
             var m=   Mobile_Transactions_Service.ReadMultiple(new MobileTransactions.Mobile_Transactions_Filter[] { new MobileTransactions.Mobile_Transactions_Filter { Criteria = loan.account.Account_No, Field = MobileTransactions.Mobile_Transactions_Fields.Account_No }, new MobileTransactions.Mobile_Transactions_Filter { Criteria = "No", Field = MobileTransactions.Mobile_Transactions_Fields.Posted } }, null, 1);
                if (m.Count() == 0)
                {
                    MobileTransactions.Mobile_Transactions t = new MobileTransactions.Mobile_Transactions();
                    t.Document_No = loan.Document_No;
                    t.Account_No = loan.account.Account_No;
                    t.Telephone_Number = loan.account.Telephone;
                    t.Amount = loan.Amount;
                    t.AmountSpecified = true;
                    t.Document_Date = DateTime.Now.Date;
                    t.Document_DateSpecified = true;
                    t.Transaction_Time = DateTime.Now;
                    t.Transaction_TimeSpecified = true;
                    t.Transaction_Type = MobileTransactions.Transaction_Type.Loan_Application;
                    t.Transaction_TypeSpecified = true;
                    Mobile_Transactions_Service.Create(ref t);
                    loan.Hasresults = true;
                    loan.results = "0";
                }
                else {
                    loan.Hasresults = false;
                    loan.Errors = "Pending Loan applications";
                }

                try { MBranch.Post(); }catch(Exception ex)  {
                    CUtilities.LogEntryOnFile(ex.Message);  }
            }
            catch (Exception ex)
            {
        ...
        [WebMethod]
        public dcs_account_details.DCS_Account_Details GetAccountDetails(string Phone)
        {
            try
            {
                Phone = '+' + Phone.Replace("+", "");
                dcs_account_details.DCS_Account_Details_Service dCSAccountDetails_Service = new dcs_account_details.DCS_Account_Details_Service ();
                dCSAccountDetails_Service.Credentials = Msacco.Credentials;
                dCSAccountDetails_Service.Url = string.Format("http://{0}:{3}/{2}/WS/{1}/Page/DCS_Account_Details", (object)this.ss.server, (object)this.ss.Companyname, (object)this.ss.Instance, (object)this.ss.Port);
                dCSAccountDetails_Service.Timeout = 60000;
                dcs_account_details.DCS_Account_Details dCSAccountDetails = dCSAccountDetails_Service.ReadMultiple(
                     new dcs_account_details.DCS_Account_Details_Filter[1]
                     {
                        new dcs_account_details.DCS_Account_Details_Filter { Field = dcs_account_details.DCS_Account_Details_Fields.MPESA_Mobile_No, Criteria=Phone }
                     },
                     null, 1)[0];
                //dcs_account_details.DCS_Account_Details dCSAccountDetails = dCSAccountDetails_Service.ReadMultiple(
                // new dcs_account_details.DCS_Account_Details_Filter[]
                // {
                //     },
                // null, 0)[0];
