...
namespace Client
{
    ...
    {        settings s = new settings();
        ...
        private void loadsettings()
        {
            try
            {
                ...
                M.Memberservice.Url = String.Format("http://{0}:{3}/{2}/WS/{1}/Page/Members", s.Serverip, s.Companyname, s.Instance, s.Port);
                M.Memberservice.PreAuthenticate = true;
                M.Memberservice.Credentials = cd;
        ...
        private void loadsettingswithcertificate()
        {
            try
            {
                ...
                M.Memberservice.Url = String.Format("http://{0}:{3}/{2}/WS/{1}/Page/Member", s.Serverip, s.Companyname, s.Instance, s.Port);
                M.Memberservice.ClientCertificates.Add(X509Certificate.CreateFromCertFile(s.certpath + "\\NavServiceCert.cer"));
                M.Memberservice.ClientCertificates.Add(X509Certificate.CreateFromCertFile(s.certpath + "\\RootNavServiceCA.cer"));
                credentialCaches.Add(new Uri(M.Memberservice.Url), "Basic", networkCredential);
                M.Memberservice.Credentials = credentialCaches;
                M.Memberservice.PreAuthenticate = true;
        ...
        public S_Applications.Applications Registration(string Phone)
        {
            return null;
        }
        ...
        {
            var m = M.getmember(telephone);
            if (m != null)
                return loan.Customerloans(m.No);
            else
                return null;
        }
        ...
        public Member.Members members(string acc)
        {
            return M.getmember(acc);
        }
    ...
    public class loan
    {
        ...
        public static List<Loans.Loans> Customerloans(string acc)
        {
            ...
            try
            {
                Customerloan = loanservice.ReadMultiple(new Loans_Filter[] { new Loans_Filter { Criteria = acc, Field = Loans_Fields.Client_Code } }, null, 0).ToList();
            }
            catch (Exception ex) { Logging.Logging.ReportError(ex); }
            return Customerloan;
        }
        ...
        internal static void LoanBalance(ref Trans t)
        {
            ...
            Trans tt = t;
            var cl = Customerloans(t.Account_No).Where(o => o.Loan_Status == Loan_Status.Issued);
            foreach (var l in cl)
            {
                loan la = new loan();
                la.loantype = l.Loan_Product_Type;
            ...
            M.member = M.getmember(a.Member_No);
            cl = Customerloans(M.member.No).Where(o => o.Loan_Status == Loan_Status.Issued);
            foreach (var l in cl)
            {
                if (ll.FirstOrDefault(o => o.loantype == l.Loan_Product_Type) == null)
                {
                    loan la = new loan();
                    la.loantype = l.Loan_Product_Type;
                    la.balance = l.Outstanding_Balance + l.Oustanding_Interest;
    ...
    public class M
    {
        public static Member.Members_Service Memberservice = new Member.Members_Service();
        public static Member.Members member;
        public static Member.Members getmember(string tel)
        {
            Member.Members m = null;
            try
            {
                
                var member = Memberservice.ReadMultiple(new Member.Members_Filter[] { new Member.Members_Filter { Criteria = tel, Field = Member.Members_Fields.MPESA_Mobile_No } },null,1);

                m = member.FirstOrDefault();
    ...
    public class Trans : Results
    {
        ...
        public static List<Accounts.Accounts> Getaccounts(string tel)
        {
            ...
            try
            {
                ...
                //3.Share capital
                //4.Christmas savings
                //5.Plaza contribution
                Accounts.Accounts a = new Accounts.Accounts();
                a.No = "1";
                a.Name = "Deposit";
                a.Account_Status = Accounts.Account_Status.Active;
                acc.Add(a);
...
{public class Depositaccounts {
    ...
    [KnownType(typeof(Member.Members))]
        ...
        {            
            get
            {
                ...
                try
                {
                    ...
                    a.Account = "Deposit";
                    a.Name = "Deposit";
                    a.Type = Member.Depositaccounts.status.savings;
                    acc.Add(a);
                    ...
                    foreach (var loan in loan.Customerloans(No))
                    {
                        a = new Depositaccounts();
                        a.Account = loan.Loan_No;
                        a.Name = loan.Loan_Product_Type;
                        a.Type = Member.Depositaccounts.status.loans;
                        acc.Add(a);
