...
namespace Client
{
    ...
    public class Members
    {
        ...
        public static Member.Member getmemberbyid(string id)
        {
            ...
            try
            {
                m = Members.Memberservice.ReadMultiple(new Member.Member_Filter[] { new Member.Member_Filter {Criteria = id, Field = Member.Member_Fields.ID_No }, new Member.Member_Filter { Criteria = "Active", Field = Member.Member_Fields.Status } },null,0).FirstOrDefault();
            }
            catch (Exception ex)
            {
                Logging.Logging.ReportError(ex);
        ...
        public List<deposits> depositbalance (string member)
        {
            ...
            try
            {
               var mm =Trans.Getaccount(member);
                if (mm != null)
                {
                    var acc = Trans.Accservice.ReadMultiple(new Accounts.Accounts_Filter[] { new Accounts.Accounts_Filter { Criteria = mm.Member_No, Field = Accounts.Accounts_Fields.Member_No } }, null, 0).Where(o => o.NWD_Account == true || o.Share_Capital_Account == true);
        ...
        public static List<Sms> sendsms(List<Sms> s)
        {
            ...
            try
            {
                foreach (var sms in s)
                {
                    try
                    {
                        Trans.s_mobile.SendSms(sms.Source, sms.Telephone_No, sms.SMS_Message, sms.Document_No);
                        sms.code = 0;
                    }
                    catch (Exception ex)
                    {
