...
class SmsController extends GetxController {
  ...
  Future<void> getsmsequity() async {
    List<SmsMessage> messages = await telephony.getInboxSms(
        ...
        // filter: SmsFilter.where(SmsColumn.ADDRESS).equals("+254723729739"),
        //filter: SmsFilter.where(SmsColumn.ADDRESS).equals("LOPHA_SACCO"),
        //.and(SmsColumn.DATE)
        //.greaterThan(DateTime(2023, 09, 01).toString()),
        sortOrder: [
    ...
    if (messages.isNotEmpty) {
      messages = messages
          .where((element) => element.body!.contains('CONFIRMED'))
          .toList();
    }
    //print(messages.length);
    gettransequity(messages);
  }
  ...
  Future<void> getsmsequity2() async {
    ...
    if (messages.isNotEmpty) {
      messages = messages
          .where((element) => element.body!.contains('Your transaction'))
          .toList();
    }
    //print(messages.length);
    gettransequity(messages);
    ...
    for (var ms in mss) {
      try {
        ...
        if (ms.body!.startsWith("CONFIRMED")) {
          recno = ms.body!
              .substring(
                  ms.body!.indexOf("Ref. ") + 5, ms.body!.indexOf(" on "))
              .replaceAll(".", "")
              .replaceAll("\n", "");
        } else {
          recno = ms.body!
              .substring(ms.body!.indexOf("Ref. ") + 5,
                  ms.body!.indexOf(". MPESA Ref"))
              .replaceAll(".", "")
              .replaceAll(" ", "");
        }
        ...
        if (ms.body!.startsWith("CONFIRMED")) {
          String? date = ms.body!.substring(
              ms.body!.indexOf(" on ") + 4, ms.body!.indexOf(" at "));
