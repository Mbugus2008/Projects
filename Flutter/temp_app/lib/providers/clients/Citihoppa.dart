...
class Cityhoppa implements BaseClients {
  ...
  Widget _buildCrewInfo(Vehicles vehicle) {
    ...
    return Obx(() {
      var crew = MemberController().getcurrentcrew(vehicle.Vehicle_Number.toString()); // Assuming currentCrew is an Rx<List<Member>?>
      ...
      return Column(
        ...
        children: [
          Text(
            ...
            style: TextStyle(
              fontSize: 12,
              color: crew[0].isDriver ? Colors.black : Colors.red, // Change color if driver is not available
            ),
          ),
          ...
          Text(
            ...
            style: TextStyle(
              fontSize: 12,
              color: crew[1].isConductor ? Colors.black : Colors.red, // Change color if conductor is not available
            ),
          ),
        ],
      );
  ...
  Widget _buildSummaryCard(VehiclesController controller) {
    final activeVehicles = controller.vehdailycollections.where((v) => v.total > 0).length;
    final totalVehicles = controller.vehdailycollections.length;
    final totalManagement = controller.vehdailycollections.fold<double>(0, (sum, v) => sum + (v.Management ?? 0));
    final totalOffload = controller.vehdailycollections.fold<double>(0, (sum, v) => sum + (v.Offload ?? 0));
    final totalAmount = controller.vehdailycollections.fold<double>(0, (sum, v) => sum + v.total);
  final cash = controller.vehdailycollections.fold<double>(0, (sum, v) => sum + (v.Cash ?? 0));
  final mpesa = controller.vehdailycollections.fold<double>(0, (sum, v) => sum + (v.Mpesa ?? 0));
        ...
        child: Column(
          ...
          children: [
            ...
            Row(
              ...
              children: [
                _buildSummaryItem('Active Vehicles', '$activeVehicles / $totalVehicles', Icons.directions_bus),
  ...
  Widget _buildSummaryItem(String label, dynamic value, IconData icon) {
    return Column(
      children: [
        ...
        SizedBox(height: 2),
        Text(
          value is double ? NumberFormat("#,##0.00", "en_US").format(value) : value.toString(),
  ...
  @override
  String? clientName = 'City Hoppa Limited';
