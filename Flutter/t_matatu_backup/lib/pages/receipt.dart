...
import 'widgets/vm_autocomplete.dart';
...
class _ReceiptState extends State<Receipt> {
  ...
  void initState() {
    ...
    _myFocusNode.addListener(() {
      if (_myFocusNode.hasFocus) {
        ...
        // Perform actions when the TextFormField gains focus
      } else {
        print('TextFormField lost focus');
        // Perform actions when the TextFormField loses focus
      }
    });
    // FocusScope.of(context).requestFocus(_myFocusNode);
    // recamount.selection =
    //     TextSelection(baseOffset: 0, extentOffset: recamount.text.length);
  }

  // Add this method to update vehicle suggestions
  void _updateVehicleSuggestions(String pattern) async {
    if (pattern.isEmpty) {
      setState(() {
    ...
    final suggestions = await memberController.getVehicleSuggestions(pattern);
    setState(() {
      _vehicleSuggestions = suggestions.map((suggestion) => VehicleSuggestion(
        vehicle: suggestion.id,
        vehicleNumber: suggestion.displayText,
        fleetNo: suggestion.isVehicle ? suggestion.details.split(', ')[0].split(': ')[1] : '',
        vehicleType: suggestion.isVehicle ? suggestion.details.split(', ')[1].split(': ')[1] : ''
      )).toList();
    });
  }
  ...
  //print(header.toJson());
    ...
    //     .value
    //     .transtions!
    //     .addAll(Get.find<HeaderController>().currTrans);
        header.Total_Amount = Get.find<HeaderController>().currTrans.fold<double>(
              0.0,
              (currentSum, item) =>
    ...
    Get.find<HeaderController>().filteredTrans.value =
        ...
        List<int>? bytes = await Get.find<MainController>().CurrentClient?.value.printReceipt(header);
        if (bytes != null)
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
              ...
              children: [
                Expanded(
                  ...
                  child: GetBuilder<TransTypeController>(
                    builder: (controller) => _buildTransTypeDropdown(controller),
                  ),
                ),
            ...
            Visibility(
              visible: Get.find<TransTypeController>().tType.value.Code == "EXPENSES",
        ...
        builder: (controller) => _buildexpensesDropdown(controller),  ),
            ...
            Row(
              ...
              children: [
                ...
                if (Get.find<MainController>().CurrentClient!.value.Auto_Assign == true)
                  ElevatedButton.icon(
                    onPressed: () { 
                      Get.find<TransTypeController>().vehicleTrantypes.forEach((element) {
  ...
  Widget _buildexpensesDropdown(ExpenseController controller) {
    List<Expenses> exps = List.from(
        Get.find<ExpenseController>().all);
    ...
    return exps.isNotEmpty
      ...
      style: const TextStyle(fontSize: 10),
      items: exps.map<DropdownMenuItem<Expenses>>(
              (Expenses? value) {
  ...
  Widget _buildTransTypeDropdown(TransTypeController controller) {
    List<TranTypes> ttypes = List.from(
        Get.find<TransTypeController>().alltrantypes);

    final first = ttypes
        .firstWhereOrNull((element) => element.Code == " ");
    if (first == null) {
    ...
    return ttypes.isNotEmpty
        ? DropdownButtonFormField<TranTypes>(
            ...
            onChanged: (TranTypes? newValue) {
              ...
              Get.find<TransTypeController>().tType.value =
                  newValue;
              Get.find<HeaderController>()
                  .amountEditingController
                  .value
                  .text = newValue.VehicleAmount.toString();
            ...
            style: const TextStyle(fontSize: 10),
            items: ttypes.map<DropdownMenuItem<TranTypes>>(
                (TranTypes? value) {
  ...
  void _handleAddEntry() {
    if (vehicleno.text.isEmpty) {
      Get.snackbar(
        ...
        duration: const Duration(
            seconds:
                3), // Set the duration the snackbar is displayed
        snackPosition: SnackPosition
            .BOTTOM, // Set the position of the snackbar
      );
      return;
    }
    ...
    if (Get.find<TransTypeController>().tType.value.Code ==
            null ||
              ...
              child: Obx(
                () => DataTable(
                  ...
                  headingTextStyle: const TextStyle(
                      ...
                      color: AppColors.textColor,
                      fontSize: 12),
                  columnSpacing: 20.0, // Set global column spacing
                  dataRowMinHeight: 10,
                  dataRowMaxHeight: 20,
                  headingRowHeight: 20,
                  columns: [
            ...
            children: [
              Autocomplete<Suggestion>(
                optionsBuilder: (TextEditingValue textEditingValue) async {
                  ...
                  return await memberController.getVehicleSuggestions(textEditingValue.text);
                },
                displayStringForOption: (Suggestion option) => option.displayText,
                onSelected: (Suggestion selection) {
                 Get.find<HeaderController>().createheader();  // Assuming this method updates the header based on current selections
                  Get.find<HeaderController>().currTrans.clear();  // Clear current transactions
                  Get.find<MainController>().vehsummary.clear();  // Clear vehicle summary
                  Get.find<MemberController>().initialize(); 
                  Get.find<HeaderController>().currHeader.value.Account = selection.account;  
                  ...
                  if (selection.isVehicle) {
                   ...
                   vehicleno.text = selection.id; 
                    memberController.getcurrentcrew(selection.displayText);  // Assuming this method sets the current crew based on vehicle ID
                    Get.find<HeaderController>().currHeader.value.Vehicle = selection.displayText;
                    Get.find<HeaderController>().currHeader.value.Fleet = selection.id;
                    Get.find<VehiclesController>().getvehtrans(selection.displayText, DateTime.now()); // Re-initialize member controller
                } 
                    ...
                    child: Material(
                      ...
                      child: ListView(
                        children: options.map((Suggestion option) {
                          print(option.displayText + "Loan: ${option.loan}");
                          return Card(
                            elevation: 2,
                            child: ListTile(
                              leading: option.isVehicle ?  const Icon( Icons.directions_bus , color: Colors.blue, size: 24):const Icon( Icons.person, color: Colors.green, size: 24),
                              title: Text(option.isVehicle ? '${option.id}-${option.displayText}' :option.displayText),
                              subtitle: Text(option.details),
                              trailing: option.loan > 0 ? Text('${NumberFormat("#,##0.00", "en_US").format(option.loan)}', style: const TextStyle(fontSize: 12, color: Colors.red),) : null,
              ...
              GetBuilder<MemberController>(
                builder: (controller) => Row(
                  ...
                  children: [
                    Expanded(flex: 2, child: _buildCrewInfo("Driver", controller.currentdriver.value)),
                    Expanded(flex: 2, child: _buildCrewInfo("Conductor", controller.currentcunductor.value)),
                    Expanded(flex: 1, child: IconButton(
                      onPressed: () => Get.to(() => CrewAssignment(vehicle: Get.find<VehiclesController>().Currentvehicle.value)),
