...
void main() {
  group('Member', () {
    ...
    test('fromMap Date_of_Birth from string', () {
      expect(
          Member.fromMap({'Date_of_Birth': '1990-05-15T00:00:00.000'})
              .Date_of_Birth,
          DateTime(1990, 5, 15));
    });
    ...
    test('fromMap nested Member_info', () {
      final mi = Member.fromMap({
        'Member_info': {'Key': 'ik1', 'Logged_In': true, 'First_Pin': '1234'}
      }).Member_info!;
      expect(mi.Key, 'ik1');
      expect(mi.Logged_In, true);
    });
    test('fromMap nested Loans', () {
      final loans = Member.fromMap({
        'Loans': [
          {'Key': 'lk1', 'Loan_No': 'L001', 'Status': 0},
          {'Key': 'lk2'}
        ]
      }).Loans!;
    ...
    test('toMap basic fields', () {
      final m =
          Member(Key: 'k1', No: 'M001', Name: 'Jane', Group_Account: false)
              .toMap();
      expect(m['Key'], 'k1');
      expect(m['Name'], 'Jane');
    ...
    test('toMap Date_of_Birth as ISO string', () {
      ...
      expect(Member(Date_of_Birth: d).toMap()['Date_of_Birth'],
          d.toIso8601String());
    });
    ...
    test('Date_of_Birth IS round-tripped', () {
      ...
      expect(
          Member.fromJson(Member(Date_of_Birth: date).toJson()).Date_of_Birth,
          date);
    });
  });
  ...
  group('member_info', () {
    test('fromMap', () {
      expect(
          member_info.fromMap(
              {'Key': 'k1', 'Logged_In': true, 'First_Pin': '5678'}).First_Pin,
          '5678');
    });
    test('default First_Pin', () {
      expect(member_info.fromMap({}).First_Pin, '');
    });
    test('round-trip', () {
      final o = member_info(
          Key: 'k1', Logged_In: true, Pin_Changed: false, First_Pin: '0000');
  ...
  group('Member enums', () {
    ...
    test('blocked', () {
      expect(blocked.Credit.index, 1);
      expect(blocked.All.index, 3);
    });
    test('status', () {
      expect(status.Active.index, 2);
      expect(status.Closed.index, 9);
    });
    test('loan status', () {
      expect(loan.status.Open.index, 0);
    });
  });
}
