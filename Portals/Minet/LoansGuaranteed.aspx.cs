...
namespace Bandari_Sacco
{
    public partial class LoansGuaranteed : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            string Logo_Path = String.Format("{0}/iansoft_logo.png", ImagePath);
            ...
            double CurrentDeposits =(double) acc.Sum(o => o.Balance); // Convert.ToDouble(MyClass.FormatNumber(MyClass.CurrentShares(Member_No_)));
            ...
            MothlySharesContribution =(double) member.Member_Subscription.FirstOrDefault(o => o.Account_Type == "S04").Monthly_Contribution;// MyClass.MothlySharesContribution(Member_No_);\
            var l = Global.loan_Service.ReadMultiple(new Mobile_Loan.Mobile_Loan_Filter[] { new Mobile_Loan.Mobile_Loan_Filter { Criteria = Session["Member_No"].ToString(), Field = Mobile_Loan.Mobile_Loan_Fields.Member_No } }, null, 0);

            TotalOutstandingLoan =(double) l.Sum(o => o.Loan_Balance);// MyClass.OutstandingLoanBalance(Member_No_);

            DateTime LastSharePayDate = DateTime.Today ;// MyClass.GetLastSharesPayDate(Member_No_);

                    string E_Mail = "", Idno_ = "", Gender = "", DoB = "", Name = "", PhoneNo = "", Branch = "", StaffNo = "", MemberNo = "";
           
                      
                        StaffNo = member.Payroll_No;// dr["Payroll No__Check No_"].ToString();
                        MemberNo = member.No;// dr["No_"].ToString();
                        E_Mail = member.E_Mail;// dr["E-Mail"].ToString();
                        Idno_ = member.National_ID_No;// dr["ID No_"].ToString();
                        Name = member.Name;// dr["First Name"].ToString() + " " + dr["Second Name"].ToString() + " " + dr["Last Name"].ToString();
                        Gender = member.Gender.ToString();// dr["Gender"].ToString();
                        PhoneNo = member.Phone_No;// dr["Mobile Phone No"].ToString() + " " + dr["Phone No_"].ToString();
            ...
            //{
                ...
                var security = Global.securities_Service.ReadMultiple(new Mobile_Securities.Mobile_Securities_Filter[] { new Mobile_Securities.Mobile_Securities_Filter { Criteria = member.No, Field = Mobile_Securities.Mobile_Securities_Fields.Code } }, null, 0);
                foreach (var loan in security)
                {
                    var loans = Global.loan_Service.Read(loan.Application_No);
                ...
                //command.Parameters.AddWithValue("@Member_No", Member_No_);
                    ...
                    Loan_No_ = loans.Loan_No;// dr["Loan  No_"].ToString();
                                             //  DateTime dt = DateTime.Now;

                    Last_Pay_Date = DateTime.Today;// MyClass.GetLoanLastPayDateWithConnection(Loan_No_, conn);

                    MemberName_ = loans.Member_Name;// dr["Name"].ToString();
                            ...
                            if (loans.Posting_Date != null)
                            {
                                dt = Convert.ToDateTime(loans.Posting_Date);
                            }
                            else if (loans.Application_Date != null)
                            {
                                dt = Convert.ToDateTime(loans.Application_Date);
                            }
                    Loan_Type = loans.Loan_Name;// dr["Product Description"].ToString().ToUpper();
                            Issue_Date = dt.ToString("dd-MM-yyyy");
                    Term = loans.Max_Installments.ToString(); // dr["Installments"].ToString();
                            End_Date_ =
                                Convert.ToDateTime(dt.AddMonths(Convert.ToInt32(loans.Max_Installments)).ToShortDateString());
                            End_Date = End_Date_.ToString("dd-MM-yyyy");
                    Approved_Amount_ =(double) loans.Principle_Amount; ;// Convert.ToDouble(dr["Approved Amount"].ToString());
                    Amont_Guaranteed =(double) loan.Amount_Guaranteed;// Convert.ToDouble(dr["Amount Guaranteed"].ToString());
                                                                      // Outstanding_Balance_ = MyClass.OutstandingSpecificLoanBalanceWithConnection(Loan_No_, conn);
                    Outstanding_Balance_ =(double) loans.Loan_Balance;// GetOustandingBalance(Loan_No_, conn);
            ...
            PdfPCell cellVerify =
                new PdfPCell(
                    new Phrase(
                        "Please verify your statement, for any discrepancies contact Iansoft Sacco Society Ltd. info@Iansoft.co.ke",
                        tableTd))
                {
                    Colspan = 12,
                    PaddingTop = 15f,
