...
namespace Coffee
{
    public partial class Debts : Form,iCoffee,IRibbon
    {
        ...
        public Debts()
        {
            try
            {
                ...
                navigation = new Navigation(stores_headerBindingSource, null, db, true, false, "", "", ribbonControl1, this);
                ...
                foreach (var item in Enum.GetValues(typeof(server.Payment_Mode)))
                    paymodeImageComboBoxEdit.Properties.Items.Add(item.ToString(), (int)item, -1);
                stores_headerBindingSource.AddNew();
            }
            catch (Exception ex) { Logging.Logging.ReportError(ex); }
        }
        public Form form => this;

        public RibbonControl Ribbon => throw new NotImplementedException();

        public Formtype formtype => Formtype.Card;

        public Form CardForm => throw new NotImplementedException();

        public void loaddata(Stores_header data)
        {
            stores_headerBindingSource.DataSource = data;
        }
        public void newitem()
        {
            ...
            sh.Posted = false;
            sh.Paymode = (int)server.Payment_Mode.Credit;
            sh.Crop_Year = coffee.setup.Current_crop;
            sh.Factory = coffee.setup.Factory;
            sh.Factory_Name = coffee.setup.Factory_Name;
            sh.Served_By = coffee.user == null ? "" : coffee.user.Name;
            db.Stores_headers.Add(sh);
            stores_headerBindingSource.DataSource = sh;
        }

        public Debts(bool newitem)
        {
            InitializeComponent();
            itemBindingSource.DataSource = db.Items.ToList();
            //storeBindingSource.DataSource = db.Stores.ToList();
        ...
        private void clientTextEdit_EditValueChanged(object sender, EventArgs e)
        {
            try
            {
                if (!clientTextEdit.Text.Equals(""))
                {
                        ...
                        limitLabel1.Text = ((double)name.Limit).ToString("N2");
                        limit_AvailableLabel1.Text = name.Stores_Limit_available.ToString("N2");
                        storesLabel1.Text = ((double)name.Total_Stores).ToString("N2");

                    }
                        if (s != null)
                        {
                            db.Stores.Remove(s);
                            storeBindingSource.Remove(s);
                        }
                ...
                switch (e.Column.Name)
                {
                    case "colItem":
                        using (var db = new AutoweighEntities(coffee.ConnectionString()))
                        {
                            ...
                            {
                                var i = db.Items.FirstOrDefault(o => o.No == e.Value.ToString());
                                if (i != null)
                                    view.SetRowCellValue(e.RowHandle, colAmount, i.Unit_Price);

                                view.SetRowCellValue(e.RowHandle, colBalance, i.Inventory - i.Inventory_Not_Synced);
        ...
        private void gridView1_FocusedRowChanged(object sender, DevExpress.XtraGrid.Views.Base.FocusedRowChangedEventArgs e)
        {
            try
            {
                ...
                {
                    totalLabel1.Text = colLine_total.SummaryItem.SummaryValue.ToString();
                    var sh = ((Stores_header)stores_headerBindingSource.Current);
                    switch ((server.Payment_Mode)sh.Paymode)
                    {
                        case server.Payment_Mode.Mpesa:

                            amount_PaidSpinEdit.Text = colLine_total.SummaryItem.SummaryValue.ToString();

                            break; 
                        case server.Payment_Mode.Both:

                            amount_PaidSpinEdit.Text = (Convert.ToDouble(colLine_total.SummaryItem.SummaryValue) - Convert.ToDouble(sh.Limit_Available)).ToString(); ;
        ...
        private void toolpost_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                stores_headerBindingSource.EndEdit();
                if ((gridView1.PostEditor() && gridView1.UpdateCurrentRow()))
                {
                    //gridView1.EndUpdate();
                    ...
                    if (d == DialogResult.Yes)
                    {
                        ...
                        colLine_total.Summary.Add(siTotal);

                        GridSummaryItem summary = colLine_total.SummaryItem;// gridView1.Columns["colLine_total"].SummaryItem;
                        sh.Total =Convert.ToDouble( summary.SummaryValue);
                        sh.Credit_Amount = sh.Total;
                        ...
                        switch ((server.Payment_Mode)sh.Paymode)
                        {
                            ...
                            case server.Payment_Mode.Credit:
                                if (sh.Limit_Available < sh.Total)
                                {
                                    sh.Credit_Amount = sh.Total;
                                    MessageBox.Show("You have no enough limit");
                                    return;
                                }
                                break;

                            case server.Payment_Mode.Both:
                                sh.Credit_Amount = sh.Total - sh.Amount_Paid;
                                sh.Collector_No = sh.Mpesa_No;
                                sh.Collector = sh.Mpesa_Name;
                                if ((sh.Limit_Available + sh.Amount_Paid??0) < sh.Total)
                                {
                                    MessageBox.Show("You have no enough limit");
                                    return;
                                }
                                if (string.IsNullOrEmpty(sh.Mpesa_Code))
                                {
                                    MessageBox.Show("Please enter the Mpesa Code");
                                    return;
                                }
                                if (string.IsNullOrEmpty(sh.Mpesa_No))
                                {
                                    MessageBox.Show("Please enter the Mpesa Number");
                                    return;
                                }if (string.IsNullOrEmpty(sh.Mpesa_Name))
                                {
                                    MessageBox.Show("Please enter the Mpesa Name");
                                    return;
                                }
                                break;
                            case server.Payment_Mode.Mpesa:
                        ...
                        {
                            ii.Sent = false;
                            ii.Amount_Paid = sh.Amount_Paid;
                           

                        }
                        sh.Item_Count = (int)sh.store_items_Qty;
                        sh.Posted = true;

                        db.SaveChanges();
                        var fr = new Reports.Storereceipt_epson(sh);
        ...
        private void storeBindingSource_AddingNew(object sender, AddingNewEventArgs e)
        {
            try
            {
                if (stores_headerBindingSource.Current != null)
                {
                    ...
                    s.Client = ss.Client;
                    s.Entry = ss.Entry;
                    s.Paymode = ss.Paymode;
                    s.Sent = false;
                    s.Served_By = coffee.user == null ? "" : coffee.user.Name;
                    s.Factory = coffee.setup.Factory;
        ...
        private void gridView1_ValidatingEditor(object sender, DevExpress.XtraEditors.Controls.BaseContainerValidateEditorEventArgs e)
        {
            ...
            var item = db.Items.Where(o => o.No == c.Item).FirstOrDefault();
            ...
            if (e.Value != null && e.Value.ToString() != string.Empty)
                switch (column.Name)
                {
                    ...
                    case "colQuantity":
                        ...
                        {
                            if (item.Prevent_Negative_Inventory == 0 || item.Prevent_Negative_Inventory == 2 )
                            {
                                e.Valid = false;
        ...
        private void Amount_PaidSpinEdit_EditValueChanged(object sender, EventArgs e)
        {
            balanceSpinEdit.Text = (Convert.ToDouble(amount_PaidSpinEdit.Value) -  Convert.ToDouble(totalLabel1.Text ==""? "0" : totalLabel1.Text)).ToString();
        }

        private void clientTextEdit_Leave(object sender, EventArgs e)
        {
            try
            {
                if (!clientTextEdit.Text.ToString().ToUpper().Contains("CASH"))
                {
                    clientTextEdit.Text = clientTextEdit.Text.PadLeft(coffee.setup.Pad_farmer_no ?? 0, '0');
                paymodeImageComboBoxEdit.SelectedIndex = 0;
                }
                else
                    paymodeImageComboBoxEdit.SelectedIndex = 1;
                ...
                if (!clientTextEdit.Text.Equals(""))
                {
                    using (var db = new AutoweighEntities(coffee.ConnectionString()))
                    {
                        ...
                        if (name != null)
                        {
                            ...
                            limitLabel1.Text = ((double)name.Limit).ToString("N2");
                            limit_AvailableLabel1.Text = name.Stores_Limit_available.ToString("N2");
                            //var unposted = name.Unposted_store;
                            storesLabel1.Text = ((double)name.Total_Stores).ToString("N2");
                        }
                        else
                        {
                            ...
                            limit_AvailableLabel1.Text = "";
                            storesLabel1.Text = "";
                            MessageBox.Show("Member not Found", "No Member", MessageBoxButtons.OK, MessageBoxIcon.Error); clientTextEdit.SelectAll();
                            clientTextEdit.Focus();
        ...
        private void collector_is_MemberCheckBox_CheckedChanged(object sender, EventArgs e)
        {
            if (collector_is_MemberCheckBox.Checked)
            {
                using (var db = new AutoweighEntities(coffee.ConnectionString()))
                {
                    var name = db.Farmers.FirstOrDefault(o => o.No == clientTextEdit.Text);
                    if (name != null)
                    {
                        collector_NoTextBox.Text = name.ID_No;
                        collectorTextBox.Text = name.Name;
                    }
                }
            }
            else
            {
                collector_NoTextBox.Text = "";
                collectorTextBox.Text = "";
            }
        }

        private void paymodeImageComboBoxEdit_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (paymodeImageComboBoxEdit.SelectedItem !=null)
            switch((server.Payment_Mode)paymodeImageComboBoxEdit.SelectedIndex)
            {
                case server.Payment_Mode.Credit:
                    panel2.Visible = true;
        ...
        private void gridView1_RowEditCanceled(object sender, RowObjectEventArgs e)
        {
            try
            {
                db.Stores.Remove((Store)e.Row);
        ...
        public void edititem<T>(T data)
        {
            ...
            if (d != null)
            {
                sh = db.Stores_headers.Where(o => o.Entry == d.Entry).FirstOrDefault();
                stores_headerBindingSource.DataSource = sh;
            }
        }
        private void storeGridControl_ProcessGridKey(object sender, KeyEventArgs e)
        {
