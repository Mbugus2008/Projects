...
namespace Coffee
{
    public partial class Collection : Form
    {
        ...
        List<String> farmerslist = new List<string>();
        public Collection()
        {
            InitializeComponent();
            ...
            try
            {
                ...
                //    {
                //        var f = dbb.Farmers.ToList();
                        
                //        foreach (var farmer in f)
                //        {
                //            farmerslist.Add(String.Join(" | ", farmer.No, farmer.Name));
                //        }
                //        if (this.txtfarmer.InvokeRequired)
                //        {
                //            this.txtfarmer.BeginInvoke((MethodInvoker)delegate ()
                //            {
                //             //   txtfarmer.Values = farmerslist.ToArray();
                //               // farmerBindingSource.DataSource = f;
                //             //   txtfarmer.AutoCompleteCustomSource = farmerslist;
                               
                //             //   txtfarmer.AutoCompleteMode = AutoCompleteMode.Suggest;
                //             //   txtfarmer.AutoCompleteSource = AutoCompleteSource.CustomSource;
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            try
            {
                using (var dbb = new AutoweighEntities(coffee.ConnectionString()))
                {
                    ...
                    if (f.Account_Category == 3)
                    {
                        if (this.delivered_ByTextBox.Text.Equals(""))
                        {
                            MessageBox.Show("delivered by should have a value");
                            delivered_ByTextBox.Focus();
                            return;
                        }
                    ...
                    c.Delivered_By = delivered_ByTextBox.Text;
                    c.Sent = false;
                    c.Collection_Number = cn;
                    c.Cancelled = "No";
                    c.Crop = coffee.setup.Current_crop;
                    c.Coffee_Type = coffee.setup.Coffe_type.ToString();
                    // c.Coffee_Type = (coffee.setup.Coffe_type == 0 ? "CHERRY" : "MBUNI");
                    switch (coffee.setup.Coffe_type)
                    {
                        case 0:
                            c.Coffe_Type_Name = "Cherry Grade One";
                            break;
                        case 1:
                            c.Coffe_Type_Name = "Cherry Grade Two";
                            break;
                        default:
                            c.Coffe_Type_Name = "Mbuni";
                            break;
                    }
                   
                    c.Factory = coffee.setup.Factory;
                    c.Gross = Convert.ToDouble(txtkg.Text);
                    c.Kg__Collected = Convert.ToDouble(netkg.Text) + Convert.ToDouble(txthold.Text);
                    c.Tare = c.Gross - c.Kg__Collected;
                    if (coffee.setup.Manual_tare == false)
                        c.No_of_Bags = Convert.ToInt32((!noofbags.Text.Equals("") ? Convert.ToDouble(noofbags.Text) : 0));
                    ...
                    if (ff != null)
                        if ((coffee.setup.Coffe_type == 0))
                        {
                           ...
                           // c.Cumm = (double)ff.Current_Crop_collection_Cherry_1 + ff.Cherry + c.Kg__Collected;
                            c.Cumm = (double) ff.Cherry_cumm + c.Kg__Collected;
                        }
                        else if ((coffee.setup.Coffe_type == 1))
                        {
                            // ff.Cum_Cherry += c.Kg__Collected;
                           // c.Cumm = (double)ff.Current_Crop_collection_Cherry_2 + ff.Cherry + c.Kg__Collected;
                            c.Cumm = (double)ff.Cherry_cumm + c.Kg__Collected;
                        }
                        ...
                        else
                        {
                            ...
                            c.Cumm = (double)ff.Mbuni_cumm + c.Kg__Collected;
                        }
     ...
     private void txtfarmer_Leave(object sender, EventArgs e)
        {
            ...
            else
            {
               ...
               txtfarmer.Text =   txtfarmer.Text.PadLeft(coffee.setup.Pad_farmer_no ?? 0, '0');
                ...
                DeliveredbyPanel.Enabled = f.Account_Category == 3;
        ...
        private void SetText(string text)
        {
            ...
            if (auto)
            {
                ...
                {
                    if (coffee.setup.Clear_Kg_on_Post == true)
                    {
                        if (!String.IsNullOrEmpty(txtfarmer.Text))
                        {
                ...
                using (var db = new AutoweighEntities(coffee.ConnectionString()))
                {
                    ...
                    if (d == DialogResult.Yes)
                    {
                        ...
                        if (coll != null)
                        {
                            ...
                            ct.Tare = coll.Tare;
                            ct.No_of_Bags = coll.No_of_Bags;
                            ct.Gross = coll.Gross;
                            ct.Coffe_Type_Name = coll.Coffe_Type_Name;
     ...
     (e.KeyChar != '.') && (e.KeyChar != '-'))
        ...
        private void delivered_ByTextBox_Leave(object sender, EventArgs e)
        {
            delivered_ByTextBox.Text = delivered_ByTextBox.Text.PadLeft(coffee.setup.Pad_farmer_no ?? 0, '0');
        }
